# Trade Log — Archive (pre-August 2026)

August 2026 onwards: see trade-log-2026-08.md
...
- **Index new-entry check:** fresh `scan` (VIX **11.74**, down from 11.91 — lowest of the day) — NIFTY spot 24,379.7 ADX **24.5** `range_bound: false`; BANKNIFTY spot 57,306.05 ADX **32.34** `range_bound: false`; SENSEX spot 78,140.57 ADX **29.54** `range_bound: false`. vs the prior read (28.69/31.67/31.49) NIFTY eased (28.69→24.5) and SENSEX eased (31.49→29.54) on a softer spot while BANKNIFTY firmed a touch (31.67→32.34) — all three still solidly trending in the mid-20s/low-30s, none within ~6 of the gate. Morning's pre-market qualifiers (NIFTY 17.41, SENSEX 17.13) firmed away all session and never returned under the gate. No `range_bound: true` on any index → per the routine, no `/trade`. **No index qualifier → no index entry.**
...
- **No trade placed or closed** → no Telegram. Capital ₹3,99,835.20 unchanged; realized from reset −₹164.80. 0 open positions — flat. **Git clean:** local HEAD and `origin/main` both at fe086b2 on read.

## 2026-07-31 intraday-monitor (latest+4) — flat, NO trade; routine re-fired again; all three indices still trending (NIFTY 28.69, BANKNIFTY 31.67, SENSEX 31.49), board firmed further into trend on a rising spot; VIX 11.91; stocks earnings-blocked; nothing to manage

`2026-07-31 ~intraday IST | — | — | NO TRADE | — | — | — | no index qualifier (all range_bound: false); stocks earnings-blocked pending steer`

- **Positions to manage on entry:** none — flat (0 open positions). `/monitor` a no-op.
- **Circuit breaker:** DISABLED in paper mode — N/A.
- **Index new-entry check:** fresh `scan` (VIX **11.91**, up a touch from 11.88) — NIFTY spot 24,405.8 ADX **28.69** `range_bound: false`; BANKNIFTY spot 57,335.85 ADX **31.67** `range_bound: false`; SENSEX spot 78,229.73 ADX **31.49** `range_bound: false`. vs the prior read (24.05/26.21/23.56) all three firmed further into trend on a rising spot — NIFTY 24.05→28.69, SENSEX 23.56→31.49, BANKNIFTY 26.21→31.67, all now solidly trending in the high-20s/low-30s. Morning's pre-market qualifiers (NIFTY 17.41, SENSEX 17.13) firmed away all session and never returned under the gate. No `range_bound: true` on any index → per the routine, no `/trade`. **No index qualifier → no index entry.**
- **Stocks:** morning's 16 qualifiers stand (daily ADX static, no mid-day re-scan). DTE fine (Aug 27 ≈ DTE 27) but earnings-blocked (peak Q1) pending Pushkar's steer → no stock entry.
- **Broker:** ✅ Dhan token valid — `funds` availBal ₹65,301.12 (util ₹934,698 = stale sid=71472 Jun2026 artifact) → confirms FLAT.
- **No trade placed or closed** → no Telegram. Capital ₹3,99,835.20 unchanged; realized from reset −₹164.80. 0 open positions — flat. **Git clean:** local HEAD and `origin/main` both at c6b0d14 on read.
...
- **Positions to manage on entry:** none — flat (0 open positions). `/monitor` a no-op.
- **Circuit breaker:** DISABLED in paper mode — N/A.
- **Index new-entry check:** fresh `scan` (VIX **11.81**, down from 11.86 — lowest of the day) — NIFTY spot 24,362.6 ADX **20.36** `range_bound: false`; BANKNIFTY spot 57,178.05 ADX **25.46** `range_bound: false`; SENSEX spot 78,041.18 ADX **19.84** `range_bound: false`. vs the prior read (19.31/26.35/18.43) NIFTY firmed further (19.31→20.36), SENSEX firmed (18.43→19.84, now ~1.8 over the gate), BANKNIFTY eased a touch (26.35→25.46). Morning's pre-market qualifiers (NIFTY 17.41, SENSEX 17.13) firmed away all session and keep drifting further above the gate. No `range_bound: true` on any index → per the routine, no `/trade`. **No index qualifier → no index entry.**
- **Stocks:** morning's 16 qualifiers stand (daily ADX static, no mid-day re-scan). DTE fine (Aug 27 ≈ DTE 27) but earnings-blocked (peak Q1) pending Pushkar's steer → no stock entry.
- **Broker:** ✅ Dhan token valid — `funds` availBal ₹65,301.12 (util ₹934,698 = stale sid=71472 Jun2026 artifact) → confirms FLAT.
- **No trade placed or closed** → no Telegram. Capital ₹3,99,835.20 unchanged; realized from reset −₹164.80. 0 open positions — flat. **Git clean:** local HEAD and `origin/main` both at 104a0ec on read.
...
- **Index scan (VIX 12.09, down a touch from 12.12):** NIFTY ADX **41.08** (spot 24,224.6) `range_bound: false`, BANKNIFTY ADX **24.26** (57,151.6) `range_bound: false`, SENSEX ADX **40.49** (77,621.64) `range_bound: false`. vs the prior read (35.67/24.23/35.26) NIFTY firmed (35.67→41.08) and SENSEX firmed (35.26→40.49) while BANKNIFTY flat (24.23→24.26) on a near-flat spot and VIX. BANKNIFTY nearest the gate at 24.26 (~6 above) but still trending. **No index qualifier → no entry.** The trend regime that firmed all session holds and is deepening (NIFTY/SENSEX now ADX ~40).
...
- **Stocks:** 22 morning qualifiers still earnings-blocked into the Jul 30 monthly, **DTE 1 (final day)** — a fresh condor at DTE 1 captures almost no theta and carries max pin/gamma; still earnings-blocked regardless (peak Q1) pending Pushkar's steer. Daily ADX static, no mid-day re-scan.
- **No trade placed/closed → no Telegram.** Capital unchanged ₹3,99,880.05; cumulative from reset −₹119.95. **Git clean:** on read, local HEAD and `origin/main` both at 4ff5574 after `git fetch origin main` — prior memory on `main`, no stranding.
...
- **`/monitor` no-op** — 0 open positions → confirmed FLAT (broker `orders`: 2 TRADED rows both the stale sid=71472 NIFTY-Jun2026-24000-CE artifact, expired 2026-06-25; no strategy legs).
- Capital ₹3,99,880.05 unchanged. No trade placed/closed → no Telegram. Git clean at 94cd570 on read.
...
- Capital ₹3,99,880.05 unchanged. No trade placed/closed → no Telegram. Git clean at 44a6cfc on read.
...
## 2026-07-27 intraday-monitor

`2026-07-27 ~open IST | NIFTY/BANKNIFTY/SENSEX | — | SKIP (both pre-market candidates firmed away) | ADX>18 all three`
...
- **Circuit breaker:** DISABLED in paper mode — N/A.
- **New-entry check:** fresh open `scan` (VIX **13.24**, down from pre-market 14.03) —
  NIFTY spot 23,946.15 ADX **21.24** `range_bound: false`; BANKNIFTY 57,198.15 ADX **32.01**
  `range_bound: false`; SENSEX 76,613.42 ADX **20.78** `range_bound: false`. **Both pre-market
  index candidates FIRMED AWAY at the open** — the recurring gate-hugger pattern: NIFTY 15.6→21.24
  and SENSEX 16.79→20.78 both climbed back above the 18 gate as spot rose (NIFTY 23,767→23,946,
  SENSEX 76,060→76,613) and VIX slipped (14.03→13.24). BANKNIFTY firmed too (25.52→32.01). **No
  index qualifier → no entry.** The "first real index setup in days" flagged pre-market did not
  survive the open — same firms-away dynamic seen 07-22 (BANKNIFTY 9.73→20.4) and 07-21
  (SENSEX 17.34→19.63).
...
## 2026-07-27 intraday-monitor (latest-2)

`2026-07-27 ~intraday IST | NIFTY/BANKNIFTY/SENSEX | — | SKIP (no setup) | ADX>18 all three`
- **Positions to manage:** none. Flat into today (portfolio authoritative in paper mode; last
  position was Position F closed 2026-07-21). `/monitor` a no-op.
- **Circuit breaker:** DISABLED in paper mode — N/A.
- **New-entry check:** fresh `scan` (VIX **13.12**, down from 13.4) — NIFTY spot 23,959.25 ADX
  **25.94** `range_bound: false`; BANKNIFTY 57,018.45 ADX **30.26** `range_bound: false`; SENSEX
  76,686.4 ADX **26.61** `range_bound: false`. All three still trending; vs the prior read
  (23.22/31.64/24.78) NIFTY firmed further (23.22→25.94) and SENSEX firmed (24.78→26.61) while
  BANKNIFTY eased slightly (31.64→30.26), spot near-flat across (NIFTY 23,897→23,959,
  BANKNIFTY 56,995→57,018, SENSEX 76,618→76,686) as VIX slipped (13.4→13.12). The pre-market
  gate-huggers (NIFTY/SENSEX) that firmed away at the open keep drifting further from the gate —
  **NIFTY nearest at 25.94, ~8 pts above 18**, still trending → no index entry.
- **Stocks:** 23 morning qualifiers still earnings-blocked (Jul 30 monthly, DTE 3, peak Q1; daily
  ADX static, no mid-day re-scan per protocol) → skip all pending Pushkar's steer.
- **No trade placed or closed** → no Telegram. Capital ₹3,99,880.05 unchanged; realized from reset
  −₹119.95. 0 open positions — flat. **Git clean:** local HEAD and `origin/main` both at f90602d on read.

## 2026-07-27 intraday-monitor (latest-3)

`2026-07-27 ~intraday IST | NIFTY/BANKNIFTY/SENSEX | — | SKIP (no setup) | ADX>18 all three`
- **Positions to manage:** none. Flat into today (portfolio authoritative in paper mode; last
  position was Position F closed 2026-07-21). `/monitor` a no-op.
- **Circuit breaker:** DISABLED in paper mode — N/A.
- **New-entry check:** fresh `scan` (VIX **13.19**, up from 13.12) — NIFTY spot 23,952.5 ADX
  **26.62** `range_bound: false`; BANKNIFTY 57,031.3 ADX **28.7** `range_bound: false`; SENSEX
  76,714.29 ADX **29.27** `range_bound: false`. All three still trending; vs the prior read
  (25.94/30.26/26.61) NIFTY firmed further (25.94→26.62) and SENSEX firmed (26.61→29.27) while
  BANKNIFTY eased slightly (30.26→28.7), spot near-flat across (NIFTY 23,959→23,953,
  BANKNIFTY 57,018→57,031, SENSEX 76,686→76,714) as VIX ticked up (13.12→13.19). The pre-market
  gate-huggers (NIFTY/SENSEX) that firmed away at the open remain well above the gate —
  **NIFTY nearest at 26.62, ~9 pts above 18**, still trending → no index entry.
- **Stocks:** 23 morning qualifiers still earnings-blocked (Jul 30 monthly, DTE 3, peak Q1; daily
  ADX static, no mid-day re-scan per protocol) → skip all pending Pushkar's steer.
- **No trade placed or closed** → no Telegram. Capital ₹3,99,880.05 unchanged; realized from reset
...
## 2026-08-03 intraday-monitor (latest+3) — flat, NO trade; all three indices still trending (NIFTY 45.9, BANKNIFTY 45.41, SENSEX 47.78), board holding deep in trend; VIX 11.95; 15 stock qualifiers earnings-blocked; nothing to manage

`2026-08-03 ~intraday IST | NIFTY/BANKNIFTY/SENSEX | ADX 45.9/45.41/47.78 | NO ENTRY (all trending, no range_bound) | flat, /monitor no-op`
- **Positions to manage:** none. Flat — 0 open positions (last position was Position G, NIFTY, EOD
  force-closed 2026-07-30 for −₹44.85). `/monitor` a no-op.
- **Circuit breaker:** DISABLED in paper mode — N/A.
- **New-entry check — no index qualifier:** fresh `scan` (VIX **11.95**) — NIFTY spot 24,586.3 ADX
  **45.9** `range_bound: false`; BANKNIFTY 57,714.0 ADX **45.41** `range_bound: false`; SENSEX
  78,775.34 ADX **47.78** `range_bound: false`. vs the prior read this session (43.26/46.44/44.67)
  the board is holding deep in trend on a near-flat spot — all three in the mid/high-40s, none within
  ~27 of the 18 gate. No `range_bound: true` → no `/trade` → **no index entry.**
- **Stocks:** this morning's 15 qualifiers stand (daily ADX static, no mid-day re-scan per protocol).
  DTE fine (nearest expiry Aug 27 monthly ≈ DTE 24, in validated DTE 2–30) but all 15 remain
...
## 2026-08-04 intraday-monitor (latest+4) — flat, NO trade; all three indices still trending (NIFTY 28.44, BANKNIFTY 23.98, SENSEX 27.38), board eased a touch further, none crossed the 18 gate; VIX 12.42; 14 stock qualifiers earnings-blocked; nothing to manage

`2026-08-04 ~intraday IST | NIFTY/BANKNIFTY/SENSEX | ADX 28.44/23.98/27.38 | NO ENTRY (all trending, no range_bound) | flat, /monitor no-op`
- **Positions to manage:** none. Flat — 0 open positions (last position was Position G, NIFTY, EOD
  force-closed 2026-07-30 for −₹44.85). `/monitor` a no-op. Broker `funds` confirms FLAT
  (availBal ₹65,301.12; util ₹934,698 = stale sid=71472 Jun2026 artifact).
- **Circuit breaker:** DISABLED in paper mode — N/A.
- **Dhan token:** valid — `funds` returned availBal ₹65,301.12.
- **New-entry check — no index qualifier:** fresh `scan` (VIX **12.42**) — NIFTY spot 24,459.85 ADX
  **28.44** `range_bound: false`; BANKNIFTY 57,402.45 ADX **23.98** `range_bound: false`; SENSEX
  78,331.47 ADX **27.38** `range_bound: false`. vs the prior read this session (30.51/25.66/27.7) all
  three eased a touch further on a softer spot — now mid/high-20s, BANKNIFTY nearest at 23.98 (~6
  above the 18 gate) but none under. No `range_bound: true` → no `/trade` → **no index entry.**
- **Stocks:** this morning's 14 qualifiers stand (daily ADX static, no mid-day re-scan per protocol).
  DTE fine (nearest expiry Aug 27 monthly ≈ DTE 23, in validated DTE 2–30) but all 14 remain
  **earnings-blocked** (tail of peak Q1) pending Pushkar's steer (2026-07-07 standing rule,
  unanswered) → no stock entry.
